var logger = require('logger').logger('password-auth');
...
module.exports = {
  direct : function (app, req, res, cbAuthed) {
    ...
    var hash = crypto.createHash('md5').update(user.toLowerCase()).digest('hex');
    ...
    function auther() {
      ...
      // allow optional existing account to be attached to
      if (req._authsome) auth.account = req._authsome.account;
      return auth;
    }
    ...
    pm.authGet(pid, app.app, function(err, auth){
      ...
      // also allowing here a mistaken registration w/ the same user and pass to
      // just succeed like a normal login, be nice
      ...
      if (req.path.indexOf("set") > 0) {
        if (app.secret !== req.param('client_secret')) {
          return  res.json(lutil.jsonErr("app not verified"), 401);
        }
