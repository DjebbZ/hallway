...
exports.ptype = 100; // must be unique per service, see dMap.js
...
exports.contact = {
  ...
  entities: function(data) { // array of any bio aspect entity name
    var ret = [];
    if(Array.isArray(data.work)) data.work.forEach(function(work){
      if(work.employer) ret.push({name:work.employer.name, type:'com'});
      if(Array.isArray(work.projects)) work.projects.forEach(function(proj){
        if(proj.name) ret.push({name:proj.name, type:'com'});
      });      
    });
    if(Array.isArray(data.education)) data.education.forEach(function(edu){
      if(edu.school) ret.push({name:edu.school.name, type:'edu'});
    });
    if(Array.isArray(data.devices)) data.devices.forEach(function(dev){
      var name = dev.os;
      if(dev.hardware) name += ' '+dev.hardware;
      ret.push({name:name, type:'dev'});
    });
  ...
  oembed: function(data) {
    ...
    if(data.username) ret.handle = data.username;
    if(data.email) ret.email = data.email;
    if(data.website) ret.website = urlnorm(data.website.split(/\s+/)[0]);
    ...
    post: function(entry) {
      ...
      if(entry.data.type) {
        if(entry.data.type !== 'status' ||
           entry.data.message           ||
           !entry.data.story            ||
           entry.data.story.indexOf('is now friends') === -1) {
...
exports.guid = {
  ...
  'post': function(entry) {
    ...
    if(entry.data.application && entry.data.application.name === 'Twitter')
    {
      ...
      if(typeof entry.data.message === 'string') return twitterGUID(handle, entry.data.message);
    }
